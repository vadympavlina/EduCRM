// ============================================================
//  shared/topbar.js
//  Дзвіночок сповіщень на всіх сторінках.
//  Викликай Topbar.init() з onAuthReady() після auth.js
// ============================================================
...
const Topbar = (() => {
  ...
  function _injectHTML() {
    // Bell button — додаємо в кінець .topbar
    const topbar = document.querySelector('.topbar');
    if (topbar && !document.getElementById('btn-notif-bell')) {
      topbar.insertAdjacentHTML('beforeend', `
        <button class="tbar-bell" id="btn-notif-bell"
                onclick="Topbar.toggle()" title="Сповіщення"
      ...
      document.body.insertAdjacentHTML('beforeend', `
        ...
        <div id="notif-panel" class="notif-panel">
          <div class="notif-panel-header">
            ...
            <button class="notif-panel-close" onclick="Topbar.close()">
              <svg width="18" height="18" fill="none" stroke="currentColor"
                   stroke-width="2.2" viewBox="0 0 24 24">
  ...
  function _renderPanel() {
    ...
    const reviews = Object.values(_allReviews)
    ...
    list.innerHTML = reviews.map(r => {
      ...
      const safeTitle   = (r.eventTitle || 'Захід')
        .replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      return '<div class="notif-item ' + (isRead ? 'read' : 'unread') + '"'
        ...
        + (url ? ' style="cursor:pointer" onclick="window.open(\'' + url + '\',\'_blank\')"' : '')
        + '>'
        ...
        + (url ? '<svg width="12" height="12" fill="none" stroke="currentColor"'
          + ' stroke-width="2.2" viewBox="0 0 24 24"'
  ...
  function _updateFavicon(hasUnread) {
    ...
    link.href = hasUnread
      ? 'data:image/svg+xml,' + encodeURIComponent(
          ...
          + '<stop offset=\'1\' stop-color=\'#3b5be8\'/></linearGradient></defs>'
          ...
          + '<text x=\'26\' y=\'11\' font-family=\'Arial\' font-size=\'9\''
